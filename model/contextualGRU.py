...
import codecs
import theano
from nn.contexualGRU_net import ContexualGRU_Net
import numpy as np
...
class ContexualGRU(object):
    ...
    def initModel(self):
        ...
        self.model = ContexualGRU_Net(X_train=self.data['X_train'], C_train=self.data['C_train'],
    ...
    def trainingModel(self):
        ...
        for epoch in range(1, self.num_epochs + 1):
            ...
            for batch in minibatches_iter(self.data['X_train'], self.data['Y_train'], masks=self.data['mask_train'],
                ...
                err = self.model.train_fn(inputs, targets, masks)
                train_err += err
                ...
                if train_batches > 0 and train_batches % self.valid_freq == 0:
                    ...
                    for batch in minibatches_iter(self.data['X_dev'], self.data['Y_dev'],
                        ...
                        inputs, targets, masks, char_inputs, lexicons = batch
                        accuracy_valid.append(self.model.eval_fn(inputs, targets, masks))
                    ...
                    if this_validation_accuracy > best_validation_accuracy:
                        ...
                        for batch in minibatches_iter(self.data['X_test'], self.data['Y_test'],
                            ...
                            inputs, targets, masks, char_inputs, lexicons = batch
                            _, preds = self.model.test_fn(inputs, targets, masks)
                            preds_test.append(preds)
                            accuracy_test.append(self.model.eval_fn(inputs, targets, masks))
            ...
            if self.update_algo != 'adadelta':
                ...
                self.model.train_fn = theano.function([self.model.input_var, self.model.target_var, self.model.mask_var],
    ...
    def testModel(self):
        ...
        for batch in minibatches_iter(self.data['X_test'], self.data['Y_test'],
            ...
            corrects = self.model.eval_fn(inputs, targets, masks)
            _, preds = self.model.test_fn(inputs, targets, masks)
            preds_test.append(preds)
